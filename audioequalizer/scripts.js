...
function start_stream_from_audio() {
  const stream_dest = audioContext.createMediaStreamDestination();
  microphone_stream = audioContext.createMediaElementSource(music);
  microphone_stream.connect(stream_dest);
...
function setup() {
  const url = "audio/music.mp3";
  music = new Audio(url);

  createCanvas(600, 400);

  //   startAudio();
  start_stream_from_audio();
